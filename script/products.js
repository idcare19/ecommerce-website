...
// DEFINED GLOBALLY to be immediately available to other scripts (especially c.js)
window.PRODUCTS = [
    {
        id: 'p08',
...
document.addEventListener('DOMContentLoaded', (event) => {
                ...
                <div class="product-card">
                    <div class="product-image-container">
                        <a href="product-detail.html?id=${product.id}">
                             <img 
                                src="${product.imgSrc}" 
                                data-hover-src="${product.imgHoverSrc}" 
    ...
    // 3. --- CATEGORY FILTER FUNCTION (Used by homepage) ---
    ...
    // 4. --- IMAGE SWAP LISTENERS ---
    function attachImageSwapListeners() {
        const images = productContainer.querySelectorAll('.product-img');
        ...
        images.forEach(img => {
            img.onmouseenter = function() {
                ...
                if(hoverSrc) {
                    this.src = hoverSrc;
                    this.setAttribute('data-hover-src', currentSrc); 
                }
            };

            img.onmouseleave = function() {
                const originalSrc = this.getAttribute('data-hover-src');
                const hoverSrc = this.src;
                if(originalSrc) {
                    this.src = originalSrc;
                    this.setAttribute('data-hover-src', hoverSrc);
                }
            };
        });
    }

    // 5. --- CARD HOVER LISTENERS ---
    ...
    if (productContainer) {
        if (typeof window.applyFilters !== 'function') {
             renderProducts(PRODUCTS); // Render all products on page load for index.html
        }
    }
});